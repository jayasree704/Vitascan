...
const REJECTION_TEXT = {
  [REJECTION.UNREADABLE]: 'This image does not belong to a test result for Vitamin D. Please choose a correct image of a test strip.',
  [REJECTION.TOO_SMALL]: 'The image resolution is too low. Please upload a clear photo of a Vitamin D test strip.',
  [REJECTION.NO_STRIP]: 'This image does not belong to a test result for Vitamin D. Please choose a correct image of a test strip.',
  [REJECTION.NO_CONTROL_LINE]: 'No control (C) line detected. This image does not belong to a test result for Vitamin D. Please choose a correct image of a test strip.',
  [REJECTION.UNEVEN_LIGHTING]: 'Shadows or glare detected on strip. Please choose a clear photo of a Vitamin D test strip.',
};
...
export function analyzePixels(value, colour, width, height) {
  ...
  // A Vitamin D cassette MUST contain white/pale neutral membrane background.
  let paleNeutralCount = 0;
  for (let i = 0; i < n; i++) {
    if (value[i] >= 110 && colour[i] <= 28) paleNeutralCount++;
  }
  if (paleNeutralCount / n < 0.08) {
    return reject(REJECTION.NO_STRIP);
  }
